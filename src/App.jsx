    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <div className="mt-4 text-center">
          ...
          <p className="text-sm text-gray-500 mt-4">{mockWaterQuality.recommendation}</p>
        </div>
...
const CheckWaterPage = () => {
    ...
    if (view === 'manual') {
        return (
            <div className="space-y-6">
                <button onClick={() => setView('main')} className="text-blue-600 font-semibold mb-4">&larr; Kembali</button>
                <h2 className="text-2xl font-bold text-gray-800" style={{fontFamily: 'Poppins, sans-serif'}}>Cek Kualitas Air Manual</h2>
                <p className="text-gray-500">Isi ciri-ciri air yang Anda amati untuk mendapatkan analisis awal.</p>
                <Card><form className="space-y-4">
                    <div><label className="block text-sm font-medium text-gray-700">Warna Air</label><select className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm"><option>Jernih</option><option>Kekuningan</option><option>Kecoklatan</option><option>Keruh</option></select></div>
                    <div><label className="block text-sm font-medium text-gray-700">Bau</label><select className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm"><option>Tidak Berbau</option><option>Bau Kaporit</option><option>Bau Besi/Logam</option><option>Bau Busuk/Comberan</option></select></div>
                    <div><label className="block text-sm font-medium text-gray-700">Endapan</label><select className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm"><option>Tidak Ada</option><option>Sedikit</option><option>Banyak</option></select></div>
                    <Button variant="primary" className="w-full">Analisis Hasil</Button>
                </form></Card>
            </div>
        )
    }
    if (view === 'photo') {
         return (
            <div className="space-y-6">
                <button onClick={() => setView('main')} className="text-blue-600 font-semibold mb-4">&larr; Kembali</button>
                <h2 className="text-2xl font-bold text-gray-800" style={{fontFamily: 'Poppins, sans-serif'}}>Upload Foto Sampel Air</h2>
                <p className="text-gray-500">Ambil atau unggah foto sampel air untuk dianalisis oleh AI kami.</p>
                <Card>
                    <div className="border-2 border-dashed border-gray-300 rounded-lg p-12 text-center"><Camera className="mx-auto h-12 w-12 text-gray-400"/><p className="mt-2 text-sm text-gray-600">Seret & lepas foto, atau klik untuk memilih</p><Button variant="secondary" className="mt-4">Pilih File</Button></div>
                    <div className="mt-4"><label className="block text-sm font-medium text-gray-700">Lokasi (otomatis dari GPS)</label><input type="text" value={mockUserData.location} className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm bg-gray-100" readOnly /></div>
                    <Button variant="primary" className="mt-6 w-full">Kirim untuk Analisis</Button>
                </Card>
            </div>
        )
    }
    if (view === 'history') {
         return (
            <div className="space-y-6">
                <button onClick={() => setView('main')} className="text-blue-600 font-semibold mb-4">&larr; Kembali</button>
                <h2 className="text-2xl font-bold text-gray-800" style={{fontFamily: 'Poppins, sans-serif'}}>Riwayat Pengecekan</h2>
                 <div className="space-y-3">{mockCheckHistory.map(item => (<Card key={item.id} className="flex items-center justify-between"><div><p className="font-bold">{item.location}</p><p className="text-sm text-gray-500">{item.date}</p></div><div className={`px-3 py-1 text-sm font-semibold rounded-full ${item.result === 'Baik' ? 'bg-green-100 text-green-800' : item.result === 'Sedang' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}>{item.result}</div></Card>))}</div>
            </div>
        )
    }
    return (
      <div className="space-y-6">
        ...
        <div className="space-y-4">
            <CheckFeatureCard icon={Pipette} title="Cek Manual" description="Masukkan ciri-ciri fisik air (warna, bau, dll) untuk analisis cepat." onClick={() => setView('manual')}/>
        ...
        <Card className="bg-blue-50"><h3 className="font-bold text-lg text-blue-800">Tanya Ahli</h3><p className="text-sm text-blue-700 mt-1">Punya pertanyaan spesifik? Diskusikan dengan para ahli atau komunitas kami.</p><Button variant="outline" className="mt-4">Buka Forum Diskusi</Button></Card>
      </div>
    );
};
    ...
    if (view === 'form') {
        return (
             <div className="space-y-6">
                ...
                <Card><form className="space-y-4">
                    <div><label className="block text-sm font-medium text-gray-700">Kategori Laporan</label><select className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm"><option>Air Tercemar/Keruh</option><option>Pipa Rusak/Bocor</option><option>Sanitasi Tidak Layak</option><option>Pembuangan Limbah Ilegal</option></select></div>
                    <div><label className="block text-sm font-medium text-gray-700">Deskripsi Masalah</label><textarea rows="4" className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm" placeholder="Jelaskan kondisi secara rinci..."></textarea></div>
                    <div><label className="block text-sm font-medium text-gray-700">Upload Foto/Video</label><div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md"><div className="space-y-1 text-center"><Camera className="mx-auto h-12 w-12 text-gray-400" /><p className="text-sm text-gray-600">Upload bukti visual</p></div></div></div>
                    <div><label className="block text-sm font-medium text-gray-700">Lokasi Masalah</label><input type="text" className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm" placeholder="Contoh: Depan Balai Kota" /></div>
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="space-y-4">
            ...
            {mockUserData.reports.length > 0 ? (mockUserData.reports.map(report => (<Card key={report.id} className="p-4"><div className="flex justify-between items-start"><div><p className="font-bold text-gray-800">{report.type}</p><p className="text-sm text-gray-500">{report.location}</p><p className="text-xs text-gray-400 mt-1">{report.date}</p></div>{getStatusChip(report.status)}</div></Card>))) : (<Card className="text-center text-gray-500">Anda belum membuat laporan.</Card>)}
        </div>
      </div>
    );
};
...
const EducationPage = () => {
    ...
    const categories = ['Semua', 'Air Minum Aman', 'Cuci Tangan', 'Sanitasi Layak', 'Konservasi Air', 'Tips Harian'];
    ...
    return (
      <div className="space-y-6">
        <h2 className="text-2xl font-bold text-gray-800 text-center" style={{fontFamily: 'Poppins, sans-serif'}}>Pusat Edukasi</h2>
        <p className="text-gray-500 text-center">Tingkatkan pengetahuanmu tentang air bersih dan sanitasi.</p>
        <div className="flex space-x-2"><div className="relative flex-grow"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" /><input type="text" placeholder="Cari artikel, video..." className="w-full pl-10 pr-4 py-2 border rounded-lg"/></div><button className="p-2 border rounded-lg"><SlidersHorizontal className="h-5 w-5 text-gray-600"/></button></div>
        <div className="flex overflow-x-auto space-x-3 pb-2 -mx-4 px-4">{categories.map(cat => (<button key={cat} onClick={() => setFilter(cat)} className={`px-4 py-2 rounded-full text-sm font-semibold whitespace-nowrap transition-colors ${filter === cat ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}>{cat}</button>))}</div>
        <div className="space-y-4">{filteredContent.map(item => (<Card key={item.id} className="flex items-center space-x-4 p-4"><div className="p-3 bg-blue-100 rounded-lg"><item.icon className="h-6 w-6 text-blue-600"/></div><div className="flex-grow"><p className="font-bold text-gray-800">{item.title}</p><div className="flex items-center space-x-2 text-xs text-gray-500 mt-1"><span className="capitalize">{item.type}</span><span>&bull;</span><span>{item.category}</span><span>&bull;</span><span>{item.audience}</span></div></div><ChevronRight className="flex-shrink-0 text-gray-400"/></Card>))}</div>
      </div>
    );
};

const ProfilePage = ({ setActiveTab, onLogout }) => {
    const stats = [
    ...
    return (
      <div className="space-y-6">
        <div className="flex flex-col items-center space-y-2">
            <img src={`https://i.pravatar.cc/150?u=${mockUserData.name}`} alt="Profile" className="w-24 h-24 rounded-full border-4 border-white shadow-lg"/>
        ...
        <Card className="p-4"><div className="grid grid-cols-3 gap-4 text-center">{stats.map(stat => (<div key={stat.label}><stat.icon className="mx-auto h-7 w-7 text-blue-600 mb-1"/><p className="text-xl font-bold text-gray-800">{stat.value}</p><p className="text-xs text-gray-500">{stat.label}</p></div>))}</div></Card>
        <div className="space-y-2">
            <h3 className="px-4 text-sm font-semibold text-gray-500">Akun</h3>
            <div className="bg-white rounded-lg shadow-sm">
                <button className="w-full text-left p-4 flex justify-between items-center"><span>Pengaturan Akun</span><ChevronRight className="h-5 w-5 text-gray-400" /></button>
                <button className="w-full text-left p-4 flex justify-between items-center border-t border-gray-100"><span>Notifikasi</span><ChevronRight className="h-5 w-5 text-gray-400" /></button>
            </div>
            <h3 className="px-4 pt-4 text-sm font-semibold text-gray-500">Aktivitas</h3>
             <div className="bg-white rounded-lg shadow-sm">
                 <button className="w-full text-left p-4 flex justify-between items-center"><span>Riwayat Laporan</span><ChevronRight className="h-5 w-5 text-gray-400" /></button>
                 <button className="w-full text-left p-4 flex justify-between items-center border-t border-gray-100"><span>Statistik Kontribusi</span><ChevronRight className="h-5 w-5 text-gray-400" /></button>
            </div>
        </div>
...
export default function App() {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'Cek Air':
        return <CheckWaterPage />;
      case 'Lapor':
        return <ReportPage />;
      case 'Edukasi':
        return <EducationPage />;
      case 'Profil':
        return <ProfilePage setActiveTab={setActiveTab} onLogout={() => setIsLoggedIn(false)} />;
      default:
        return <HomePage setActiveTab={setActiveTab} />;
    }
  };
